# @dramaorg/nam-pariatur

`@dramaorg/nam-pariatur` is a cache for asynchronous fetching of resources
with full deduplication, i.e. the same resource is only asked once at any given time.

## Install

```bash
npm i @dramaorg/nam-pariatur
```

## Example

```js
import { createCache } from '@dramaorg/nam-pariatur'
...
```js
import { createCache, createStorage } from '@dramaorg/nam-pariatur'
...
```ts
import { createCache, Cache } from "@dramaorg/nam-pariatur";
...
```html
<script src="https://unpkg.com/@dramaorg/nam-pariatur"></script>
